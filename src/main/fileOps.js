...
function resolveDestinationPath(destinationDir, fileName) {
  ...
  while (true) {
    ...
    counter++
    if (counter > 9999) {
      throw new Error(`Terlalu banyak file duplikat untuk "${fileName}" di "${destinationDir}"`)
    }
  }
}
...
function ensureDestinationReady(sourcePath, destinationDir, rule) {
  ...
  if (!fs.existsSync(destinationDir)) {
    if (rule.autoCreateFolder !== false) {
      createDirectoryIfNotExists(destinationDir)
    } else {
      throw new Error(`Folder tujuan tidak ada dan autoCreateFolder dinonaktifkan: "${destinationDir}"`)
    }
  }
}
...
function scanAndSort(sourceDirectory, rules) {
  ...
  for (const fileEntry of files) {
    ...
    for (const rule of activeRules) {
      ...
      try {
        const logEntry = rule.action === 'copy'
          ? copyFile(sourcePath, rule.destination, rule)
          : moveFile(sourcePath, rule.destination, rule)
...
function undoAction(logEntry) {
  ...
  if (!fs.existsSync(logEntry.to)) {
    throw new Error(`File tidak ditemukan di lokasi tujuan — mungkin sudah dihapus atau dipindahkan manual: "${logEntry.to}"`)
  }
  ...
  if (logEntry.action === 'copy') {
    ...
    if (!fs.existsSync(originalDir)) {
      try {
        ...
        throw new Error(`Tidak bisa membuat ulang folder asal "${originalDir}": ${err.message}`)
      }
    }

    if (fs.existsSync(logEntry.from)) {
      throw new Error(`File sudah ada di lokasi asal — tidak bisa dikembalikan tanpa menimpa: "${logEntry.from}"`)
    }

    try {
      _renameCrossDevice(logEntry.to, logEntry.from)
    } catch (err) {
      throw new Error(`Gagal mengembalikan file "${logEntry.fileName}" ke lokasi asal: ${err.message}`)
    }
  }
