    ...
    rules: {
      ...
      items: {
        ...
        properties: {
          ...
          organizeBy:       { type: 'string' },
          destinationBase:  { type: 'string' },
          conflictStrategy: { type: 'string', default: 'rename' },
          duplicateStrategy:{ type: 'string', default: 'none' },
          priority:         { type: 'number', default: 100 },
          autoCreateFolder: { type: 'boolean', default: true },
          isActive:         { type: 'boolean', default: true },
        },
        required: ['id', 'watchFolder'],
      },
    },
    logs: {
      type: 'array',
...
function trayTooltip() {
  ...
  if (status.isActive && status.watchingFolders.length > 0) return `AutoMover — Memantau ${status.watchingFolders.length} folder`
  return 'AutoMover — Nonaktif'
}

function safeSendToRenderer(channel, payload) {
  try {
    if (mainWindow && !mainWindow.isDestroyed() && mainWindow.webContents) mainWindow.webContents.send(channel, payload)
  } catch {}
}
...
function buildTrayMenu() {
  ...
  return Menu.buildFromTemplate([
    ...
    { label: isMonitoring ? `● Memantau ${watcherManager.watchers.size} folder` : '○ Monitoring Nonaktif', enabled: false },
    {
      ...
      click: async () => {
        if (isMonitoring) { store.set('settings.autoMonitor', false); watcherManager.stop() }
        else { const rules = store.get('rules'); store.set('settings.autoMonitor', true); watcherManager.start(rules, mainWindow) }
      ...
      click: async () => {
        ...
        for (const folder of folders) {
          try {
            ...
            for (const logEntry of result.success) { persistLogEntry(logEntry); safeSendToRenderer('watcher:fileProcessed', logEntry) }
          } catch (err) { console.error('[Tray RunNow]', err.message) }
        }
      },
    },
    { type: 'separator' },
...
function createWindow() {
  ...
  mainWindow.on('close', (event) => {
    ...
    event.preventDefault(); mainWindow.hide(); if (process.platform === 'darwin') app.dock?.hide()
    if (!settings.trayHintShown) {
      store.set('settings.trayHintShown', true)
      if (Notification.isSupported()) new Notification({ title: 'AutoMover masih berjalan', body: 'Aplikasi tetap aktif di system tray. Klik dua kali ikon tray untuk membuka kembali.', silent: false }).show()
    }
    refreshTray()
  })
...
function createTray() {
  ...
  tray.setToolTip(trayTooltip())
  tray.setContextMenu(buildTrayMenu())
  tray.on('double-click', () => { mainWindow?.show(); mainWindow?.focus(); if (process.platform === 'darwin') app.dock?.show() })
  tray.on('click', () => { if (process.platform !== 'darwin') tray.popUpContextMenu() })
}

ipcMain.handle('rules:getAll', () => store.get('rules'))
ipcMain.handle('rules:save', (_event, rules) => { store.set('rules', rules); return { success: true } })
...
ipcMain.handle('watcher:runNow', async () => {
  ...
  const allResults = { success: [], skipped: [], errors: [] }
  const folders = [...new Set(activeRules.map(r => r.watchFolder).filter(Boolean))]
  for (const folder of folders) {
    try {
      const result = scanAndSort(folder, activeRules)
      ...
      for (const logEntry of [...result.success, ...(result.skipped || [])]) { persistLogEntry(logEntry); safeSendToRenderer('watcher:fileProcessed', logEntry) }
    } catch (err) { allResults.errors.push({ folder, error: err.message }) }
  }
  return allResults
})

ipcMain.handle('preview:run', async () => {
  try { const { previewRules } = _require('./fileOps'); return previewRules(store.get('rules')) }
  catch (err) { return { success: false, generatedAt: new Date().toISOString(), items: [], errors: [{ error: err.message }], summary: { planned: 0, error: 1, createFolders: 0, conflicts: 0, renamed: 0, folders: 0 } } }
})
...
ipcMain.handle('store:saveRules', (_event, rules) => { store.set('rules', rules); return { success: true } })
