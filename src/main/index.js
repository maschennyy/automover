...
function buildTrayMenu() {
  ...
  return Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    {
      label: isMonitoring ? `● Memantau ${watcherManager.watchers.size} folder` : '○ Monitoring Nonaktif',
      enabled: false,
    },
    {
...
ipcMain.handle('preview:run', async () => {
  try {
    const { previewRules } = _require('./fileOps')
    const rules = store.get('rules')
    return previewRules(rules)
  } catch (err) {
    return {
      success: false,
      ...
      summary: { planned: 0, error: 1, createFolders: 0, conflicts: 0, renamed: 0, folders: 0 },
    }
  }
})
...
ipcMain.handle('settings:update', (_event, partial) => {
  ...
  if (partial.runAtStartup !== undefined) app.setLoginItemSettings({ openAtLogin: partial.runAtStartup })
...
ipcMain.handle('app:saveSettings', async (_event, settings) => {
  ...
  if (settings.runAtStartup !== undefined) app.setLoginItemSettings({ openAtLogin: settings.runAtStartup })
