...
function EmptyState({ onRun, busy, compact }) {
  return (
    <div style={{ height: '100%', display: 'grid', placeItems: 'center', padding: compact ? 18 : 32 }}>
      <div style={{ maxWidth: 500, textAlign: 'center', padding: compact ? 20 : 26, background: 'var(--bg-surface)', border: '1px dashed var(--border-strong)', borderRadius: 'var(--radius-xl)' }}>
        <div style={{ fontSize: compact ? 34 : 42, marginBottom: 12 }}>◌</div>
        <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: compact ? 17 : 20, margin: 0 }}>No preview yet</h3>
        {!compact && <p style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.65 }}>Run preview untuk melihat rencana file tanpa memindahkan apa pun.</p>}
        <ActionButton onClick={onRun} disabled={busy} primary compact={compact}>{busy ? 'Scanning...' : 'Run Preview'}</ActionButton>
      </div>
    </div>
  )
}
    ...
    <div className="panel-scroll" style={{ height: '100%' }}>
      <table style={{ width: 'max-content', minWidth: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead style={{ position: 'sticky', top: 0, zIndex: 1, background: 'var(--bg-surface)' }}>
          <tr>{['Status', 'File', 'Rule', 'Action', 'Destination', 'Note'].map(head => <th key={head} style={{ textAlign: 'left', color: 'var(--text-muted)', fontSize: 11, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '.06em', padding: '10px 12px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-surface)' }}>{head}</th>)}</tr>
        </thead>
        <tbody>
          {items.map(item => {
...
export default function PreviewPage({ compact = false }) {
  ...
  const executePreview = async () => {
    ...
    if (!window.confirm(`Execute ${planned} planned action${skipped ? ` dan ${skipped} skip` : ''}?`)) return
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, minWidth: 0, display: 'grid', gridTemplateColumns: compact ? '1fr' : 'minmax(0, 1fr) 300px', overflow: 'hidden' }}>
      <section style={{ display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: 0, overflow: 'hidden' }}>
        <div style={{ display: 'flex', flexDirection: compact ? 'column' : 'row', alignItems: compact ? 'stretch' : 'flex-start', justifyContent: 'space-between', gap: compact ? 12 : 18, padding: compact ? '16px 18px 14px' : '20px 24px 16px', borderBottom: '1px solid var(--border-subtle)', flexShrink: 0 }}>
          <div><div style={{ color: 'var(--accent)', fontSize: 10, fontWeight: 900, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 5 }}>Preview</div><h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: compact ? 20 : 23, margin: 0 }}>Dry Run</h1>{!compact && <p style={{ margin: '7px 0 0', color: 'var(--text-secondary)', fontSize: 13 }}>Simulate first, execute after review.</p>}</div>
          <div style={{ display: 'flex', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}><ActionButton compact={compact} onClick={() => exportPreviewCsv(result)} disabled={!canExport}>Export</ActionButton><ActionButton compact={compact} onClick={executePreview} disabled={!canExecute || executing || busy} danger>{executing ? 'Executing...' : 'Execute'}</ActionButton><ActionButton compact={compact} onClick={runPreview} disabled={busy || executing} primary>{busy ? 'Scanning...' : 'Run Preview'}</ActionButton></div>
        </div>
        <div style={{ padding: compact ? 14 : 18, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10, borderBottom: '1px solid var(--border-subtle)', flexShrink: 0 }}><StatCard compact={compact} label="Planned" value={summary.planned} hint="ready" tone="planned" /><StatCard compact={compact} label="Skipped" value={summary.skipped || 0} hint="skip" tone={summary.skipped ? 'warning' : 'neutral'} /><StatCard compact={compact} label="Errors" value={summary.error} hint="check" tone={summary.error ? 'danger' : 'neutral'} /><StatCard compact={compact} label="Folders" value={summary.createFolders} hint="create" tone="warning" /><StatCard compact={compact} label="Conflicts" value={summary.conflicts} hint="found" tone={summary.conflicts ? 'warning' : 'neutral'} /><StatCard compact={compact} label="Renamed" value={summary.renamed} hint="suffix" tone={summary.renamed ? 'warning' : 'neutral'} /></div>
        {error && <div style={{ margin: compact ? 14 : 18, padding: 12, background: '#ef444418', border: '1px solid #ef444444', color: '#f87171', borderRadius: 'var(--radius)', fontSize: 12 }}>{error}</div>}
        {notice && <div style={{ margin: compact ? 14 : 18, padding: 12, background: '#22c55e18', border: '1px solid #22c55e44', color: '#4ade80', borderRadius: 'var(--radius)', fontSize: 12 }}>{notice}</div>}
      ...
      {!compact && <aside style={{ borderLeft: '1px solid var(--border-subtle)', background: 'var(--bg-surface)', overflow: 'hidden', minHeight: 0 }}><PreviewInspector result={result} /></aside>}
    </div>
  )
}
