...
function duplicateLabel(value) {
  if (value === 'skip-same-name-size') return 'Skip same name+size'
  return 'None'
}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)', padding: '24px 28px', width: 340, boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, margin: '0 0 20px' }}>Aturan <strong style={{ color: 'var(--text-primary)' }}>"{ruleName || 'Tanpa Nama'}"</strong> akan dihapus permanen. File yang sudah dipindahkan tidak terpengaruh.</p>
...
function Chip({ children, tone = 'accent' }) {
  ...
  return <span style={{ fontSize: 10, fontWeight: 800, padding: '2px 7px', borderRadius: 4, background: palette[0], color: palette[1], border: `1px solid ${palette[2]}`, letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>{children}</span>
}

function ControlLabel({ children }) {
  return <label style={{ color: 'var(--text-muted)', fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', whiteSpace: 'nowrap' }}>{children}</label>
}

function SelectControl({ value, onChange, children, title }) {
  return <select value={value} onChange={onChange} title={title} style={{ background: 'var(--bg-overlay)', border: '1px solid var(--border-default)', color: 'var(--text-primary)', borderRadius: 7, padding: '6px 9px', fontSize: 12, fontFamily: 'DM Sans, sans-serif', outline: 'none' }}>{children}</select>
}
...
function RuleCard({ rule, onEdit, onDelete }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ background: hovered ? 'var(--bg-elevated)' : 'var(--bg-surface)', border: `1px solid ${rule.isActive ? 'var(--border-default)' : 'var(--border-subtle)'}`, borderRadius: 'var(--radius-lg)', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 11, opacity: rule.isActive ? 1 : 0.55, transition: 'background 0.15s, border-color 0.15s, opacity 0.2s' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button type="button" onClick={() => toggleRule(rule.id)} title={rule.isActive ? 'Nonaktifkan' : 'Aktifkan'} style={{ width: 36, height: 20, borderRadius: 99, border: '1px solid var(--border-default)', background: rule.isActive ? 'var(--accent)' : 'var(--bg-overlay)', position: 'relative', cursor: 'pointer', flexShrink: 0 }}><span style={{ position: 'absolute', top: 2, left: rule.isActive ? 17 : 2, width: 14, height: 14, borderRadius: '50%', background: '#fff', transition: 'left 0.18s' }} /></button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono, monospace', background: 'var(--bg-overlay)', padding: '7px 10px', borderRadius: 6, overflow: 'hidden' }}>
        <span title={rule.watchFolder} style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{truncatePath(rule.watchFolder)}</span><span>→</span><span title={rule.destination || 'Folder sumber'} style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{rule.destinationBase === 'source' ? 'Folder sumber' : truncatePath(rule.destination)}</span>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
        ...
        <Chip tone={duplicate === 'skip-same-name-size' ? 'amber' : 'neutral'}>Duplicate: {duplicateLabel(duplicate)}</Chip>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 88px auto minmax(160px, 220px)', alignItems: 'center', gap: 8 }}>
        <ControlLabel>Priority</ControlLabel>
        <input type="number" min="1" max="999" value={priority} onChange={e => updatePartial({ priority: Number(e.target.value || 100) })} title="Priority kecil diproses lebih dulu" style={{ width: 88, background: 'var(--bg-overlay)', border: '1px solid var(--border-default)', color: 'var(--text-primary)', borderRadius: 7, padding: '6px 9px', fontSize: 12, fontFamily: 'DM Sans, sans-serif', outline: 'none' }} />
        <ControlLabel>Duplicate</ControlLabel>
        <SelectControl value={duplicate} onChange={e => updatePartial({ duplicateStrategy: e.target.value })} title="Duplicate detection">
          <option value="none">None</option>
          <option value="skip-same-name-size">Skip same name + size</option>
        </SelectControl>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <ControlLabel>Jika nama sama</ControlLabel>
        <SelectControl value={conflict} onChange={e => updatePartial({ conflictStrategy: e.target.value })} title="Conflict strategy">
          <option value="rename">Rename otomatis</option><option value="skip">Skip</option><option value="overwrite">Overwrite</option>
        </SelectControl>
        <span style={{ color: 'var(--text-muted)', fontSize: 11 }}>{conflict === 'rename' && 'file baru diberi suffix _1, _2, dst.'}{conflict === 'skip' && 'file bentrok dilewati.'}{conflict === 'overwrite' && 'file lama di tujuan akan ditimpa.'}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '20px 24px 14px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div><h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, margin: 0 }}>Aturan Sortir</h1><p style={{ color: 'var(--text-secondary)', fontSize: 12, marginTop: 3 }}>{rules.length === 0 ? 'Belum ada aturan' : `${rules.length} aturan · ${rules.filter(r => r.isActive).length} aktif · urut priority`}</p></div>
        <button type="button" onClick={onAddNew} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--accent)', border: 'none', borderRadius: 'var(--radius)', color: '#fff', fontSize: 13, fontWeight: 700, padding: '8px 14px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}><IconPlus /> Tambah Aturan</button>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
        {rules.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 14, color: 'var(--text-muted)', paddingBottom: 60 }}><div style={{ fontSize: 48, lineHeight: 1 }}>📂</div><div style={{ textAlign: 'center' }}><p style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-secondary)' }}>Belum ada aturan</p><p style={{ fontSize: 12, marginTop: 4 }}>Klik "+ Tambah Aturan" untuk membuat aturan sortir pertama</p></div><button type="button" onClick={onAddNew} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--accent-muted)', border: '1px dashed var(--accent-border)', borderRadius: 'var(--radius)', color: 'var(--accent)', fontSize: 13, fontWeight: 700, padding: '9px 18px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', marginTop: 4 }}><IconPlus /> Buat Aturan Pertama</button></div>
