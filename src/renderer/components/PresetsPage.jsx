...
function modeLabel(mode) { if (mode === 'category') return 'Category'; if (mode === 'extension') return 'Extension'; if (mode === 'name') return 'Name'; return 'Fixed' }
...
function PresetCard({ preset, active, onClick, compact }) {
  ...
  return <button type="button" onClick={onClick} style={{ textAlign: 'left', padding: compact ? 13 : 15, borderRadius: 'var(--radius-xl)', border: active ? '1.5px solid var(--accent)' : '1px solid var(--border-subtle)', background: active ? 'var(--accent-muted)' : 'var(--bg-surface)', color: active ? 'var(--accent)' : 'var(--text-primary)', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', display: 'flex', flexDirection: 'column', gap: 10, minWidth: 0 }}><div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', minWidth: 0 }}><div style={{ width: compact ? 34 : 38, height: compact ? 34 : 38, borderRadius: 11, background: active ? 'rgba(124,58,237,.22)' : 'var(--bg-elevated)', display: 'grid', placeItems: 'center', fontSize: compact ? 18 : 21, flexShrink: 0 }}>{preset.icon}</div><div style={{ minWidth: 0 }}><div style={{ fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 800, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{preset.title}</div>{!compact && <div style={{ color: active ? 'var(--accent)' : 'var(--text-secondary)', opacity: active ? 0.88 : 1, fontSize: 12, lineHeight: 1.5, marginTop: 4 }}>{preset.desc}</div>}</div></div><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}><Pill tone="accent">{modeLabel(preset.rule.organizeBy)}</Pill><Pill tone="green">{preset.rule.action}</Pill>{extCount > 0 ? <Pill>{extCount} ext</Pill> : <Pill tone="amber">all</Pill>}{preset.rule.filters.namePattern && <Pill tone="amber">name</Pill>}{preset.id === 'reference-list' && <Pill tone="amber">txt/csv</Pill>}</div></button>
}

function SummaryRow({ label, value }) { return <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: 10, padding: '9px 0', borderBottom: '1px solid var(--border-subtle)' }}><div style={{ color: 'var(--text-muted)', fontSize: 10, fontWeight: 900, letterSpacing: '.06em', textTransform: 'uppercase' }}>{label}</div><div style={{ color: 'var(--text-primary)', fontSize: 12, lineHeight: 1.5, wordBreak: 'break-word' }}>{value || '-'}</div></div> }
...
function SetupPanel({ compact, selected, watchFolder, setWatchFolder, destination, setDestination, destinationBase, setDestinationBase, referenceText, setReferenceText, referenceItems, isReferencePreset, customDestinationRequired, error, notice, busy, browse, importReferenceFile, createPresetRule }) {
  return <div className="panel-scroll" style={{ padding: compact ? 14 : 18, display: 'flex', flexDirection: 'column', gap: 14, height: '100%' }}><div><div style={{ fontFamily: 'Syne, sans-serif', fontSize: 16, fontWeight: 800 }}>Setup</div>{!compact && <p style={{ color: 'var(--text-secondary)', fontSize: 12, lineHeight: 1.55 }}>Pilih folder sebelum membuat rule.</p>}</div><div><label style={{ color: 'var(--text-muted)', fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '.06em' }}>Source</label><div style={{ display: 'flex', gap: 8, marginTop: 7 }}><input value={watchFolder} onChange={e => setWatchFolder(e.target.value)} placeholder="Source folder" style={{ flex: 1, minWidth: 0, background: 'var(--bg-overlay)', border: '1px solid var(--border-default)', color: 'var(--text-primary)', borderRadius: 'var(--radius)', padding: '9px 10px', fontSize: 12, outline: 'none' }} /><ActionButton compact={compact} onClick={() => browse(setWatchFolder)}>Browse</ActionButton></div></div><div><label style={{ color: 'var(--text-muted)', fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '.06em' }}>Destination</label><div style={{ display: 'flex', gap: 8, marginTop: 7, flexWrap: 'wrap' }}><ActionButton compact={compact} onClick={() => setDestinationBase('source')} primary={destinationBase === 'source'}>Source</ActionButton><ActionButton compact={compact} onClick={() => setDestinationBase('custom')} primary={destinationBase === 'custom'}>Custom</ActionButton></div></div>{customDestinationRequired && <div><label style={{ color: 'var(--text-muted)', fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '.06em' }}>Custom folder</label><div style={{ display: 'flex', gap: 8, marginTop: 7 }}><input value={destination} onChange={e => setDestination(e.target.value)} placeholder="Destination folder" style={{ flex: 1, minWidth: 0, background: 'var(--bg-overlay)', border: '1px solid var(--border-default)', color: 'var(--text-primary)', borderRadius: 'var(--radius)', padding: '9px 10px', fontSize: 12, outline: 'none' }} /><ActionButton compact={compact} onClick={() => browse(setDestination)}>Browse</ActionButton></div></div>}{isReferencePreset && <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}><label style={{ color: 'var(--text-muted)', fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '.06em' }}>Reference List</label><textarea value={referenceText} onChange={e => setReferenceText(e.target.value)} placeholder={'1. IMG_7434\n2. IMG_ 7458\n3. IMG_7478'} rows={compact ? 6 : 8} style={{ resize: 'vertical', background: 'var(--bg-overlay)', border: '1px solid var(--border-default)', color: 'var(--text-primary)', borderRadius: 'var(--radius)', padding: '9px 10px', fontSize: 12, outline: 'none', fontFamily: 'JetBrains Mono, monospace', lineHeight: 1.5 }} /><div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}><label style={{ padding: '8px 12px', border: '1px solid var(--border-default)', borderRadius: 'var(--radius)', background: 'var(--bg-surface)', color: 'var(--text-secondary)', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>Import<input type="file" accept=".txt,.csv,text/plain,text/csv" onChange={importReferenceFile} style={{ display: 'none' }} /></label><span style={{ color: 'var(--text-muted)', fontSize: 11 }}>{referenceItems.length} keywords</span></div></div>}{error && <div style={{ padding: 11, background: '#ef444418', border: '1px solid #ef444444', borderRadius: 'var(--radius)', color: '#f87171', fontSize: 12, lineHeight: 1.5 }}>{error}</div>}{notice && <div style={{ padding: 11, background: '#22c55e18', border: '1px solid #22c55e44', borderRadius: 'var(--radius)', color: '#4ade80', fontSize: 12, lineHeight: 1.5 }}>{notice}</div>}<div style={{ padding: 13, background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)' }}><SummaryRow label="Preset" value={selected.title} /><SummaryRow label="Mode" value={modeLabel(selected.rule.organizeBy)} /><SummaryRow label="Action" value={selected.rule.action} /><SummaryRow label="Ext" value={selected.rule.filters.extensions.length ? selected.rule.filters.extensions.join(', ') : 'All'} /><SummaryRow label="Name" value={isReferencePreset ? `${referenceItems.length} keyword` : selected.rule.filters.namePattern || '-'} /></div><ActionButton compact={compact} onClick={createPresetRule} disabled={busy} primary>{busy ? 'Creating...' : 'Create Rule'}</ActionButton></div>
}
