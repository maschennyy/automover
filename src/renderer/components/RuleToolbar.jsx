...
function SelectControl({ value, onChange, children }) {
  return <select value={value} onChange={onChange} style={{ minWidth: 0, background: 'var(--bg-overlay)', border: '1px solid var(--border-default)', color: 'var(--text-primary)', borderRadius: 7, padding: '7px 8px', fontSize: 12, fontFamily: 'DM Sans, sans-serif', outline: 'none' }}>{children}</select>
}

function SmallButton({ children, onClick, disabled, primary = false }) {
  return <button type="button" onClick={onClick} disabled={disabled} style={{ padding: '8px 10px', border: primary ? 'none' : '1px solid var(--border-default)', borderRadius: 'var(--radius)', background: disabled ? 'var(--bg-overlay)' : primary ? 'var(--accent)' : 'var(--bg-surface)', color: disabled ? 'var(--text-muted)' : primary ? '#fff' : 'var(--text-secondary)', fontSize: 12, fontWeight: 800, cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'DM Sans, sans-serif', whiteSpace: 'nowrap' }}>{children}</button>
}
...
function normalizeImportedRules(payload) {
  ...
  return source.filter(rule => rule && typeof rule === 'object' && rule.watchFolder).map(rule => ({
    ...rule,
    id: uuidv4(),
    name: rule.name || 'Imported Rule',
    filters: { extensions: Array.isArray(rule.filters?.extensions) ? rule.filters.extensions : [], namePattern: rule.filters?.namePattern || '' },
...
export function matchesRuleToolbarFilters(rule, filters) {
  ...
  const searchOk = !q || [rule.name, rule.watchFolder, rule.destination, rule.action, rule.organizeBy, rule.filters?.namePattern, ...(rule.filters?.extensions || [])].filter(Boolean).some(value => String(value).toLowerCase().includes(q))
...
export default function RuleToolbar({ rules, shownCount, activeCount, filters, setFilters, onAddNew, onImportRules }) {
  ...
  const importRules = async (event) => {
    ...
    try {
      ...
      if (!imported.length) throw new Error('No valid rules found in this file')
      await onImportRules(imported)
      setImportMessage(`${imported.length} rule imported.`)
    ...
    <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: 9, flexShrink: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          <label style={{ padding: '8px 10px', border: '1px solid var(--border-default)', borderRadius: 'var(--radius)', background: 'var(--bg-surface)', color: 'var(--text-secondary)', fontSize: 12, fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap' }}>Import<input type="file" accept="application/json,.json" onChange={importRules} style={{ display: 'none' }} /></label>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(140px, 1fr) 120px 110px auto', gap: 8, alignItems: 'center' }}>
        ...
        <SelectControl value={filters.status} onChange={event => setFilters({ ...filters, status: event.target.value })}><option value="all">All status</option><option value="active">Active</option><option value="inactive">Inactive</option></SelectControl>
        <SelectControl value={filters.action} onChange={event => setFilters({ ...filters, action: event.target.value })}><option value="all">All action</option><option value="move">Move</option><option value="copy">Copy</option></SelectControl>
        <SmallButton onClick={clearFilters} disabled={!hasFilter}>Clear</SmallButton>
      </div>
      {importMessage && <div style={{ color: '#4ade80', fontSize: 12 }}>{importMessage}</div>}
      {importError && <div style={{ color: '#f87171', fontSize: 12 }}>{importError}</div>}
    </div>
  )
}
